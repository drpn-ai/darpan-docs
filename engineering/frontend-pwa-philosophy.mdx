...
Darpan's frontend is the user control surface for the backend engine; these decisions explain where the browser surface ends and backend ownership begins.

The app is implemented in [`drpn-ai/darpan-ui`](https://github.com/drpn-ai/darpan-ui). It helps users set up connections, define schemas and RuleSets, start reconciliation runs, and reopen saved output without moving that work into the browser. The current PWA direction is about shell behavior, route structure, keyboard access, responsive layout, and durable browser workflows. It is not currently an offline-first or installable app.

## What PWA means today

| Area | Current behavior |
| --- | --- |
| App shell | `src/App.vue` owns the shared shell, floating actions, user menu, read-only active-tenant indicator, theme toggle, and command palette. The actual tenant switch control lives on the **User Settings** page (`src/pages/settings/UserSettingsPage.vue`). |
...
See also: [Frontend design system](/engineering/frontend-design-system) for shared CSS, component, and page-surface contracts; [Engineering process](/engineering/process) for how frontend changes are traced and verified.
