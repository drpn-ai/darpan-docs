...
Darpan API references focus on verified backend facade contracts exposed through Moqui JSON-RPC. Every method below is defined in the Darpan backend component under `service/facade/*.xml` (or a named integration component) and is called as the JSON-RPC `method` value.
...
| Method | Purpose |
| --- | --- |
| `search#NavigationTargets` | Power Ask Darpan search over pages, workflows, records, runs, and outputs. |
...
| --- | --- | --- |
| `shopify-darpan` | `facade.ShopifyFacadeServices` | `list#ShopifyAuthConfigs`, `get#ShopifyAuthConfig`, `save#ShopifyAuthConfig`, `delete#ShopifyAuthConfig`, `list#ShopifySourceDefinitions`, `get#ShopifySourceDefinition` |
