...
A RuleSet is the comparison logic Darpan applies after it matches records across two sources. Each rule compares one field from the first source against one field from the second source. When a matched pair fails a rule, Darpan records a difference, which is what surfaces a sync issue for review and notifications.

Rules live with the saved run setup, so a run and its RuleSet are managed together. You author and edit rules in the **Ruleset Manager**.
...
Open **Ask Darpan** with `Cmd/Ctrl+K` or the floating **Ask Darpan** button. Search for **Ruleset Manager** and open the result for the saved run you are configuring.
...
Select an operator box to edit that rule, or add a new rule.

- Choose the **Field** from the first source.
- Choose the **Operator**.
- Choose the matching field from the second source.
