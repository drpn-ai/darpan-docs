...
Search for **Open Tenant Settings**. Confirm the tenant timezone and notification status.

Use [Manage tenant settings](/guides/manage-tenant-settings) when the timezone or Google Chat run-completion webhook needs to change.
...
- **Open Shopify Settings** and **Open OMS Settings** for API-backed order reconciliation.

Use active records with clear names so runs and automations are easy to recognize later.
...
- SFTP, NetSuite, Shopify, and OMS records use clear tenant-specific names.
...
Ask a Super Admin to create users, assign tenant access, change role permissions, configure AI providers, change platform settings, or investigate private operational diagnostics.
